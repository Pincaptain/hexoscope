class Chip {
    ...
    switch (this.type) {
      case STANDARD:
        if (selectedChip && selectedChip.id === this.id) {
          if (this.isActive) {
            context.fillStyle = selectedActiveChipFillStyle;
            break;
          }
        ...
        if (this.canBeSwapped()) {
          context.fillStyle = canBeSwappedChipFillStyle;
          break;
        }
  ...
  contains(x, y) {
    return this.x <= x && x <= this.x + chipWidth && this.y <= y && y <= this.y + chipHeight;
  }
}